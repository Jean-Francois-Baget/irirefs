...
import fr.inria.boreal.jfbaget.nanoparse.readers.OptionalReader;
import fr.inria.boreal.jfbaget.nanoparse.readers.RegexReader;
import fr.inria.boreal.jfbaget.nanoparse.readers.RepetitionReader;
import fr.inria.boreal.jfbaget.nanoparse.readers.SequenceReader;
import fr.inria.boreal.jfbaget.nanoparse.readers.StringReader;
...
public class IRIParser extends Parser{
	...
	private static final String CHARS__IPRIVATE = "\\uE000-\\uF8FF" + "\\x{F0000}-\\x{FFFFD}" + "\\x{100000}-\\x{10FFFD}";
	...
	private static final String REGEXP__ISEGMENT_NZ_NC = "([" + CHARS__IUNRESERVED + CHARS__SUB_DELIMS + "@]|" + REGEXP__PCT_ENCODED + ")+";
	
	private static final String REGEXP__IREG_NAME = "([" + CHARS__IUNRESERVED + CHARS__SUB_DELIMS + "]|" + REGEXP__PCT_ENCODED + ")*";
	...
	private static final String REGEXP__IPVFUTURE = "v[0-9A-Fa-f]+\\.([" + CHARS__UNRESERVED + CHARS__SUB_DELIMS + ":])+";
	...
	private static final String PATTERN__IPV6ADDRESS = "(H:){6}(L)|::(H:){5}(L)|(H)?::(H:){4}(L)|" +
	                                                   "((H:){0,1}H)?::(H:){3}(L)|" +
	                                                   "((H:){0,2}H)?::(H:){2}(L)|" +
	                                                   "((H:){0,3}H)?::H:(L)|" +
	                                                   "((H:){0,4}H)?::(L)|" + 
	...
	private static final String REGEXP__IPV6ADDRESS = PATTERN__IPV6ADDRESS.replaceAll("H", REGEXP__H16).replaceAll("L", REGEXP__LS32);
	...
	private static List<IReader> readers() {
		return List.of(
			...
			new SequenceReader(IRI, List.of(SCHEME, ":", "ihier_part", "_opt_iquery", "_opt_ifragment"), false, List.of(0, 2, 3, 4)),
			new SequenceReader(ABSOLUTE, List.of(SCHEME, ":", "ihier_part", "_opt_iquery"), false, List.of(0, 2, 3)),
			...
			new RegexReader("ifragment", REGEXP__IFRAGMENT, false),
			new RegexReader("iquery", REGEXP__IQUERY, false),
			new ChoiceReader("ihier_part", List.of("_seq_ihier_part", "ipath_absolute", "ipath_rootless", "ipath_empty"), false),
			new SequenceReader("_seq_ihier_part", List.of("//", "iauthority", "ipath_abempty"), false, List.of(1, 2)),
			new StringReader("ipath_empty", "", false),
			new ChoiceReader("irelative_part", List.of("_seq_ihier_part", "ipath_absolute", "ipath_noscheme", "ipath_empty"), false),
			new StringReader("//", "//", false),
			...
			new RepetitionReader("ipath_abempty", "_seq_ipath_abempty", null, 0, Integer.MAX_VALUE, false),
			...
			new SequenceReader("IP_literal", List.of("[", "_choice_IP_literal", "]"), false, 1),
			new StringReader("[", "[", false),
			new StringReader("]", "]", false),
			new ChoiceReader("_choice_IP_literal", List.of("IPv6address", "IPvFuture"), false),
	...
	public IRIParser() {
		super(readers());
		this.setDefaultReaderName("IRI_reference");
	}
